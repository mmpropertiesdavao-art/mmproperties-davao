...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (parsed.featuredPosition !== null) await clearFeaturedPosition(parsed.featuredPosition, id);
    const { rows } = await db.query({ text: `UPDATE blog_posts SET title=$1,slug=$2,category=$3,content=$4,content_json=$5::jsonb,cover_image_url=$6,excerpt=$7,seo_title=$8,meta_description=$9,published_at=$10,is_featured=$11,featured_position=$12,updated_at=now() WHERE id=$13::uuid RETURNING id,slug`, values: [parsed.title,slug,parsed.category,parsed.content,JSON.stringify(parsed.blocks),parsed.coverImageUrl,parsed.excerpt,parsed.seoTitle,parsed.metaDescription,parsed.publishedAt,parsed.featuredPosition!==null,parsed.featuredPosition,id] });
...
async function clearFeaturedPosition(position: number, excludeId?: string) {
  await db.query({
    text: `UPDATE blog_posts SET featured_position=NULL,is_featured=false,updated_at=now() WHERE featured_position=$1::int AND ($2::uuid IS NULL OR id<>$2::uuid)`,
    values: [position, excludeId || null],
  });
}
