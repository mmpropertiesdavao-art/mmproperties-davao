...
export default async function PropertyPage({
  ...
  const [{ rows: amenities }, { rows: images }, { rows: videos }] = await Promise.all([
    ...
    db.query<{ id: string; url: string; videoType: string }>({
      text: `SELECT id,url,video_type AS "videoType" FROM property_videos WHERE property_id=$1 ORDER BY id`,
      values: [property.id],
    }),
  ]);
  ...
  const propertyLocation = `${property.barangay || property.neighborhoodName}, Davao City`;
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2">
          ...
          {property.previousPrice && property.previousPrice > property.price && (
            <p className="mt-3 text-sm font-semibold text-red-600">
              <span className="line-through">
                PHP {property.previousPrice.toLocaleString("en-PH")}
              </span>{" "}
              · PRICE DOWN
            </p>
          )}
          ...
          <p className="mt-1 text-3xl font-semibold">
            PHP {property.price.toLocaleString("en-PH")}
          </p>

          {property.listingIntent !== "sale" && property.rentPrice && (
            <p className="mt-1 text-xl font-semibold text-sky-700">
              PHP {property.rentPrice.toLocaleString("en-PH")}/month
            </p>
          )}

          {property.propertyType === "lot-only" && property.lotAreaSqm && (
            <p className="mt-1 text-sm text-gray-500">
              PHP {Math.round(property.price / property.lotAreaSqm).toLocaleString("en-PH")}/sqm
            </p>
          )}
          ...
          <div className="mt-6 rounded-lg border border-navy-100 bg-white p-4 text-sm">
            <p className="font-semibold text-navy-900">
              Listed by {listedByName}
            </p>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-navy-100 bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-sm text-navy-500">
              Ask about price, availability, payment terms, or schedule a viewing.
            </p>

            <PropertyContactModal
              propertyTitle={property.title}
              propertyPrice={formatPeso(property.price)}
