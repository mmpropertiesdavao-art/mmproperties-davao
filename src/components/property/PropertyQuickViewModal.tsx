...
function getListedByName(property: Property) {
  const anyProperty = property as Property & {
    listedByName?: string | null;
    listedByRole?: string | null;
    carport?: number | null;
  };

  if (anyProperty.listedByName) return anyProperty.listedByName;
...
function getCarport(property: Property) {
  const anyProperty = property as Property & { carport?: number | null };

  if (anyProperty.carport !== undefined && anyProperty.carport !== null) {
    return anyProperty.carport;
  }

  return property.parkingSpaces ?? null;
}
...
export function PropertyQuickViewModal({
  ...
  const images = useMemo(() => {
    ...
    if (fallbackImageUrl) {
      return [{ url: fallbackImageUrl, altText: fallbackTitle }];
    }

    return [{ url: "/placeholder-property.png", altText: fallbackTitle }];
          ...
          {!loading && property && (
            <div className="p-5 lg:p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-y border-navy-100 py-4">
                ...
                {getCarport(property) !== null && (
                  <SpecItem icon={<Car size={15} className="text-navy-700" />}>
                    {getCarport(property)} carport
                  </SpecItem>
                )}
              </div>
